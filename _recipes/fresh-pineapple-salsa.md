---
layout: default
title: Fresh Pineapple Salsa
title_en: Fresh Pineapple Salsa
title_ja: パイナップルサルサ
genre: sauces
order: 1
---

<!-- ===================== -->
<!-- English -->
<!-- ===================== -->
<div class="lang" data-lang="en">
  <h1 class="title-recipe">🍍 Fresh Pineapple Salsa</h1>

  <!-- Ingredients -->
  <div class="container">
    <h2 class="title-ingredient">Ingredients</h2>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">Fresh pineapple</div>
      <div class="column-right-amount">1 medium</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">Red bell pepper</div>
      <div class="column-right-amount">1</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">Red onion</div>
      <div class="column-right-amount">1/2 small, chopped</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">Lime juice</div>
      <div class="column-right-amount">3 tbsp (1 1/2 lime)</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">Sea salt</div>
      <div class="column-right-amount">1/4 tspn</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">(Optional) Fresh cilantro</div>
      <div class="column-right-amount">1/4 cup chopped</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">(Optional) Jalapeño</div>
      <div class="column-right-amount">1 medium</div>
    </div>
  </div>

  <!-- Instructions -->
  <div class="container">
    <h2 class="title-instruction">Instructions</h2>
    <ol>
      <li>Grill pineapple, pepper, and jalapeño. Remove from grill.</li>
      <li>Chop pineapple medium. Add to medium sized bowl.</li>
      <li>Remove pepper skin and seeds. Chop red pepper medium, jalapeño finely. Add to bowl.</li>
      <li>Add onion, cilantro, and lime juice. Mix well.</li>
      <li>Season with salt to taste.</li>
    </ol>
  </div>
</div>

<!-- ===================== -->
<!-- Japanese -->
<!-- ===================== -->
<div class="lang" data-lang="ja">
  <h1 class="title-recipe">🍍 パイナップルサルサ</h1>

  <!-- 材料 -->
  <div class="container">
    <h2 class="title-ingredient">材料</h2>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">パイナップル</div>
      <div class="column-right-amount">1個（中）</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">赤パプリカ</div>
      <div class="column-right-amount">1個</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">赤たまねぎ</div>
      <div class="column-right-amount">1/2個（小）、みじん切り</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">ライム果汁</div>
      <div class="column-right-amount">大さじ 3 (1.5個)</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">シーソルト</div>
      <div class="column-right-amount">小さじ 1/4</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">(任意) パクチー</div>
      <div class="column-right-amount">60mL、みじん切り</div>
    </div>
    <div class="ingredient-quantity">
      <div class="column-left-ingredient">(任意) ハラペーニョ</div>
      <div class="column-right-amount">1個（中）/div>
    </div>
  </div>

  <!-- 作り方 -->
  <div class="container">
    <h2 class="title-instruction">作り方</h2>
    <ol>
      <li>パイナップル、パプリカ、ハラペーニョをグリルで焼き、取り出す。</li>
      <li>パイナップルを中くらいの大きさに切り、中サイズのボウルに入れる。</li>
      <li>パプリカの皮と種を取り除き、中くらいに刻む。ハラペーニョは細かく刻み、ボウルに加える。</li>
      <li>玉ねぎ、パクチー、ライム果汁を加え、よく混ぜる。</li>
      <li>塩で味を調える。</li>
    </ol>
  </div>
</div>
